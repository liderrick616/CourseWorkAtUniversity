...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
    ...
    // Get the index of the source ingredient
    if(k<=dist)
    {
        return h;
    }
    int source_pos = ingredient_index(source_ingredient);
    if (source_pos == -1)
    {
        return h;
    }
    for (int i = 0; i <MAT_SIZE; i++)
    {
        // Check if the ingredient is a neighbour of the source ingredient
        if (AdjMat[source_pos][i] > 0)
        {
            if(searchInt(h,i)==0){
                h= insertInt(h,i);
            }
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid) {
    ...
    int avoid_index = ingredient_index(avoid); // Get index of avoid ingredient
    intNode *related_to_source = related_k_dist(NULL, source_ingredient, k_source, 0);
    intNode *related_to_avoid = related_k_dist(NULL, avoid, k_avoid, 0);
    intNode *result = NULL;

    if (avoid_index == -1) //ingredient not found
    {
        return result;
    }
    while (related_to_source != NULL) {
        if (!searchInt(related_to_avoid, related_to_source->x) && related_to_source->x != avoid_index) {
            result = insertInt(result, related_to_source->x);
        }
        related_to_source = related_to_source->next;
    }
    deleteList(related_to_source);
    deleteList(related_to_avoid);
...
{
    int n = 0; // number of ingredients in the recipe
    while (strlen(recipe[n]) > 0 && n < 10) {
        n++;
    }

    // Find the index of the ingredient to change in the recipe
    int to_change_idx = -1;
    for (int i = 0; i < n; i++) {
    ...
    if (to_change_idx == -1) {
        // Ingredient to change not found in recipe
        return;
    }
    int compatibility_scores[MAT_SIZE];
    memset(compatibility_scores, 0, sizeof(compatibility_scores));
    for (int i = 0; i < MAT_SIZE; i++) {
        for (int j = 0; j < n; j++) {
            int idx = -1;
            for (int k = 0; k < MAT_SIZE; k++) {
                if (strcmp(ingredients[k], recipe[j]) == 0 && strcmp(ingredients[k],recipe[to_change_idx])!= 0) {
                    idx = k;
                    break;
                }
            }
            if (idx != -1) {
                compatibility_scores[i] += AdjMat[idx][i];
            }
        }
    }
    int max_score = -1;
    int max_score_idx = -1;
    for (int i = 0; i < MAT_SIZE; i++) {
        if (compatibility_scores[i] > max_score) {
            max_score = compatibility_scores[i];
            max_score_idx = i;
        }
    }
    if (max_score_idx == -1) {
        // No compatible ingredient found, don't change anything
        return;
    }

    // Replace the ingredient in the recipe
    strcpy(recipe[to_change_idx], ingredients[max_score_idx]);
